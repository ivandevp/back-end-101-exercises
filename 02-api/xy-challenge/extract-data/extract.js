...
const actionPuppeteer = async link => {
  const browser = await puppeteer.launch();
  const page = await browser.newPage();

  await page.goto(link);

  const divsCounts = await page.$$eval("ul > li.s-result-item", funkoSections =>
    funkoSections.map((section, index) => {
      let price = section.querySelector(
        "div > div > div > a > span.a-size-base"
      );
      ...
      const obj = {
        ...
        price: price ? price.innerText : "200",
        star: star ? Number(star.innerText.match(/-?\d+\.?\d*/)[0]) : 0,
...
const arrayFinal = links.map(actionPuppeteer);
console.log(arrayFinal);


Promise.all(arrayFinal).then(value => {
  //console.log(value);
  const funkosData = value.reduce((a, b) => a.concat(b));
